...
extern int expiration=0;                      // expiration in minutes for pending orders
...
int start(){
   ...
   if(ZZ>0 && ZZ!=EMPTY_VALUE && ZZ<=Low[ZZbar] && ZZ!=instance && ZZ!=firstbarvalue
   && openbuy){if(reversesignals)sell=true;else buy=true;instance=ZZ;}
   
   if(ZZ>0 && ZZ!=EMPTY_VALUE && ZZ>=High[ZZbar] && ZZ!=instance && ZZ!=firstbarvalue
   && opensell){if(reversesignals)buy=true;else sell=true;instance=ZZ;}

   if(bc!=Bars){tpb=0;bc=Bars;}
   //Comment("\nZZ  =  " + DoubleToStr(ZZ,4),"\ninstance  =  " + DoubleToStr(instance,4));
   ...
   if((count(OP_BUY,magic)+count(OP_SELL,magic))<maxtrades){  
      if(buy && tpb<tradesperbar && IsTradeAllowed()){
         ...
         RefreshRates();ticket=OrderSend(Symbol(),OP_BUYSTOP,ilots,Ask+distance*pt,slippage*mt,sl,tp,comment+". Magic: "+DoubleToStr(magic,0),magic,expire,Blue);
      ...
      if(sell && tpb<tradesperbar && IsTradeAllowed()){
         ...
         if(martingale)ilots=NormalizeDouble(lots*martingalefactor(),2);else ilots=lots;
         if(ilots<minlot)ilots=minlot;if(ilots>maxlot)ilots=maxlot;
         RefreshRates();ticket=OrderSend(Symbol(),OP_SELLSTOP,ilots,Bid-distance*pt,slippage*mt,sl,tp,comment+". Magic: "+DoubleToStr(magic,0),magic,expire,Red);
         if(ticket<=0){Print("Error Occured : "+errordescription(GetLastError()));}
