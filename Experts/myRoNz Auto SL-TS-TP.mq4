...
//|                                                                  |
//+------------------------------------------------------------------+
  enum ENUM_MONITORPROFIT_ENABLE
  {
   MP_DISABLE=0,//Disable
   MP_ENABLE=1,//Enable
  };
//+------------------------------------------------------------------+
//|                                                                  |
...
sinput string note5="";//-=[ MONITOR PROFIT  SETTINGS ]=-
input ENUM_MONITORPROFIT_ENABLE MonitorProfitEnable=LP_ENABLE;//Enable/Disable Profit Lock
   ...
   for(int i=0;i<OrdersTotal();i++)
     {
      ...
      if(SLnTPMode==Server)
        {
         ...
         if(OrderStopLoss()==0.0 && OrderTakeProfit()==0.0 && MonitorProfitEnable == True)
            bool res=OrderModify(OrderTicket(),OrderOpenPrice(),SL,TP,0,Blue);
         else if(OrderTakeProfit()==0.0 && MonitorProfitEnable == True)
      ...
      //Hidden SL and TP
        ...
        {
         if((TakeProfit>0 && Points>=TakeProfit && MonitorProfitEnable == True)  || (StopLoss>0 && Points<=-StopLoss))
           {
