//------------------------------------------------------------------------------------------------+
//                                                                                                |
//                                      CalendarFX-I-A11.mq4                                      |
//  facilitates link to Time2x                                                                    |
//------------------------------------------------------------------------------------------------+
...
// added "NewsLineDescription" to turn Object Descriptions for NewsLilnes On/Off
...
extern int    NextUpXCoordinate=10;
extern int    NextUpYCoordinate=15;
extern bool   ShowLineText=true;
...
extern bool   HistoryOn=False;
extern int    SleepInSeconds=30;
		...
		if (!skip)
         ...
         if ((StringFind(mainData[newsIdx][TITLE], "Bank Holiday", 
			...
			if (Alert_Minutes_Before != 0 && minsTillNews == Alert_Minutes_Before && numM1Bars!=iBars(Symbol(),PERIOD_M1))
			{
				...
				if (AlertOn) Alert(Alert_Minutes_Before, " minutes until news for "+ mainData[newsIdx][COUNTRY], ": ", mainData[newsIdx][TITLE]);
				if (NotificationOn) SendNotification(Alert_Minutes_Before+" minutes until news for "+mainData[newsIdx][COUNTRY]+ ": "+ mainData[newsIdx][TITLE]); 
...
void OutputToChart()
   ...
   else if (dispMins < 60) {TimeStr = dispMins + "m ";intNextUpEvent=4;}
   ...
   ObjectCreate(News4, OBJ_LABEL, W, 0, 0);
		if((index != -1) || (FLAG_none3)) 
	      ...
	      if (intNextUpEvent==4) strNextUpEvent=TimeStr + sinceUntil + dispCountry[3] + ": " + dispTitle[3];
   ...
   else if (dispMins < 60) {TimeStr = dispMins + "m ";intNextUpEvent=3;}
	else // time is 60 minutes or more
	   {
	   intNextUpEvent=3;
		Hours = MathRound(dispMins / 60);
   ...
   ObjectCreate(News3, OBJ_LABEL, W, 0, 0);
		if((index != -1) || (FLAG_none2)) 
	      ...
	      if (intNextUpEvent==3) strNextUpEvent=TimeStr + sinceUntil + dispCountry[2] + ": " + dispTitle[2];
   ...
   else if (dispMins < 60) {TimeStr = dispMins + "m ";intNextUpEvent=2;}
	else // time is 60 minutes or more
	   {
	   intNextUpEvent=2;
	   Hours = MathRound(dispMins / 60);
   ...
   ObjectCreate(News2, OBJ_LABEL, W, 0, 0);
		if((index != -1) || (FLAG_none1)) 
	      ...
	      if (intNextUpEvent==2) strNextUpEvent=TimeStr + sinceUntil + dispCountry[1] + ": " + dispTitle[1];
	...
	else if (dispMins < 60) {TimeStr = dispMins + "m ";intNextUpEvent=1;}
	else // time is 60 minutes or more
	   {
	   intNextUpEvent=1;
		Hours = MathRound(dispMins / 60);
	...
	ObjectCreate(News1, OBJ_LABEL, W, 0, 0);
		if((index != -1) || (FLAG_none0)) 
		   ...
		   if (intNextUpEvent==1) strNextUpEvent=TimeStr + sinceUntil + dispCountry[0] + ": " + dispTitle[0];
	   	ObjectSetText(News1, TimeStr + sinceUntil + dispCountry[0] + ": " + dispTitle[0], TxtSize, Text_Style, ImpactToColor(dispImpact[intNextUpEvent-1]));
      ...
      if (ShowNextUp)
            ...
            ObjectSetText("strNextUp",strNextUpEvent, TxtSize, Text_Style, ImpactToColor(dispImpact[intNextUpEvent-1]));
  ...
  {   
     ...
     if (TimeCurrent()<=timeSeconds1p || HistoryOn) {SetTLine(0,"LineNews1",0,timeSeconds1p,lineHigh1,timeSeconds1p,lineLow1,news1Color,NewsLineStyle,NewsLineWidth);   //  +Period()*60
     ...
     if (TimeCurrent()<=timeSeconds2p || HistoryOn) {SetTLine(0,"LineNews2",0,timeSeconds2p,lineHigh2,timeSeconds2p,lineLow2,news2Color,NewsLineStyle,NewsLineWidth);
     ...
     if (TimeCurrent()<=timeSeconds3p || HistoryOn) {SetTLine(0,"LineNews3",0,timeSeconds3p,lineHigh3,timeSeconds3p,lineLow3,news3Color,NewsLineStyle,NewsLineWidth);
  ...
  if(ShowLineText==true) 
  {
   ...
   if(Show_Panel_Background && ShowTable==true) {    
   ...
   int curXnew=curX;
   if(ShowTable!=true) {curXnew=-1000;}//////////////////////////////////////
   //Draw Event4
