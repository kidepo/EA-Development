...
#property strict
#property description "2 TimeFrame Matrix Chart of the ASCTrend1 indicator."
...
int start() {
  ...
  if (tCurBar!=Time[0] || iDrawLines>PDLine_Bars) {
    ...
    for (i=MathMin(iDrawLines,MathMin(PDLine_Bars,Bars-1));i>=0;i--) {
      if (iBarShift(Symbol(), giTimeFrame[IDX_R2], Time[i]) != iBarShift(Symbol(), giTimeFrame[IDX_R2], Time[i+1]) && PDLine2_Size>0)
