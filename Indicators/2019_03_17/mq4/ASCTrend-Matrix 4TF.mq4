/*------------------------------------------------------------------------------------
   Name: ASCTrend-Matrix 4TF.mq4
   
   Description: Four timeframe ASCTrend Matrix.
...
extern color     PDLine4_Color=White;       /* Period Divider Line TF4 Color */ // Color of the TF4 period divider line
extern int       PDLine4_Style=STYLE_SOLID; /* Period Divider Line TF4 Style */ // Style of the TF4 period divider line: SOLID=0, DASH=1, DOT=2, DASHDOT=3, DASTDOTDOT=4
extern int       PDLine4_Size=2;            /* Period Divider Line TF4 Size */ // Size of the TF4 period divider line. Set to 0 to not show the line.
...
int init() {
  ...
  if (TimeFrame_Auto) {
    ...
    giTimeFrame[IDX_R4]=NextHigherTF(giTimeFrame[IDX_R3]); 
    giRepaintBars[IDX_R4]=giTimeFrame[IDX_R4]/Period()+2;
  }
  else {
    ...
    giRepaintBars[IDX_R3]=giTimeFrame[IDX_R3]/Period()+2;
    giTimeFrame[IDX_R4]=TimeFrame4_Period;
    giRepaintBars[IDX_R4]=giTimeFrame[IDX_R4]/Period()+2;
  }  
...
int start() {
  ...
  // Timeframe 4
  // Set bars to redraw
  iNewBars=MathMax(RedrawBars(giTimeFrame[IDX_R4],IDX_R4,iCountedBars),giRepaintBars[IDX_R4]);
  iDrawLines=MathMax(iDrawLines,iNewBars);
  // Calc indicator data and update matrix
  ProcessASCTrend(iNewBars, giTimeFrame[IDX_R4], ASCTrend_Risk, 0, gadR4Up, gadR4Dn,IDX_R4);
  ...
  if (tCurBar!=Time[0] || iDrawLines>PDLine_Bars) {
    ...
    Writelabel(TF2Str(giTimeFrame[IDX_R4]),gadGap[IDX_R4]+0.2,Time[0]+Period()*60*2);

    // Update time-frame divider lines
    for (i=MathMin(iDrawLines,MathMin(PDLine_Bars,Bars-1));i>=0;i--) {
      //PrintD("Start");
      if (iBarShift(Symbol(), giTimeFrame[IDX_R4], Time[i])!=iBarShift(Symbol(), giTimeFrame[IDX_R4], Time[i+1]) && PDLine4_Size>0)
        DrawDividerLine(i,i,PDLine4_Style,PDLine4_Size,PDLine4_Color);
      else if (iBarShift(Symbol(), giTimeFrame[IDX_R3], Time[i]) != iBarShift(Symbol(), giTimeFrame[IDX_R3], Time[i+1]) && PDLine3_Size>0)
