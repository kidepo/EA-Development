/*
   ...
   Website: h T tP:/ /WW w. me t A q UotE s .N E T
   E-mail : S U PP Or T @me T aqUOTe s.nEt
*/
#property copyright "BO_M2M5_Alert.mq4"
   ...
   if (G_count_104 == 0) {
      if (EnableSoundAlert) {
         if (G_ibuf_92[0] != EMPTY_VALUE && G_ibuf_92[0] > 0.0) {
            Alert(TimeToStr(Time[0], TIME_DATE|TIME_MINUTES) + " BO_Alert Dot sell " + Symbol() + " " + f0_0(Period()));
            G_count_104++;
            if (EnableMailAlert) {
               SendMail(TimeToStr(Time[0], TIME_DATE|TIME_MINUTES) + " BO_Alert Dot sell " + Symbol() + " " + f0_0(Period()), TimeToStr(Time[0], TIME_DATE|TIME_MINUTES) + " BO_Alert Dot sell " +
                  Symbol() + " " + f0_0(Period()));
            }
         }
         if (G_ibuf_96[0] != EMPTY_VALUE && G_ibuf_96[0] > 0.0) {
            Alert(TimeToStr(Time[0], TIME_DATE|TIME_MINUTES) + " BO_Alert Dot buy " + Symbol() + " " + f0_0(Period()));
            G_count_104++;
            SendMail(TimeToStr(Time[0], TIME_DATE|TIME_MINUTES) + " BO_Alert Dot buy " + Symbol() + " " + f0_0(Period()), TimeToStr(Time[0], TIME_DATE|TIME_MINUTES) + " BO_Alert Dot buy " +
               Symbol() + " " + f0_0(Period()));
         }
      }
   }
